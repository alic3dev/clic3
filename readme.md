...
- - `clic3_pathing_base`: returns a path without the ending path part, also known as the base of a file path, or, if the path is at it's final part then return `./` if the path is not an absolute path otherwise return `/`
